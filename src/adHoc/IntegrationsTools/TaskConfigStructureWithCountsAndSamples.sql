...
(
    select
        ...
        R.StringValue,
        R.Type,
        PathValue = json_value(STCGT.StepConfig, R.JsonPath collate SQL_Latin1_General_CP1_CI_AS)
    from
        #Result R
        join dbo.atbl_Integrations_ScheduledTasksConfigGroupTasks STCGT with (nolock)
            on STCGT.PrimKey = R.ConfigRef
) T
group by
    T.StepType,
    T.JsonPath,

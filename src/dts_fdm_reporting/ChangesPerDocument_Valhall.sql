...
from (
    ...
    where
        ...
        and (
            ...
            or */isnull(D.PlantID, '') <> isnull(I.DCS_PlantID, '')
            or isnull(D.OriginatorCompany, '') <> isnull(I.DCS_OriginatorCompany, '')
            or isnull(D.FacilityID, '') <> isnull(I.DCS_FacilityID, '')
            or isnull(D.DocumentType, '') <> isnull(I.DCS_DocumentType, '')
            or isnull(D.DocumentGroup, '') <> isnull(I.DCS_DocumentGroup, '')
            or isnull(D.Discipline, '') <> isnull(I.DCS_Discipline, '')
            or isnull(D.ContractNo, '') <> isnull(I.DCS_ContractNo, '')
            or isnull(D.Criticality, '') <> isnull(I.DCS_Criticality, '')
            -- or isnull(D.Area, '') <> isnull(I.DCS_Area, '')
            ...
            -- or isnull(D.System, '') <> isnull(I.DCS_System, '')
            -- or isnull(D.VoidedDate, '') <> isnull(I.DCS_VoidedDate, '')
            -- or isnull(D.ReviewClass, '') <> isnull(I.DCS_ReviewClass, '')
        )
) T
order by
    Domain,

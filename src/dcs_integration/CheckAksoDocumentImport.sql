...
(
    ...
    from
        ...
        left join dbo.ltbl_Import_ProArc_Documents as ID with (nolock)
            on ID.document_number = Q.DocumentID
        left join dbo.ltbl_Import_ProArc_Revisions as IR with (nolock)
            on IR.document_number = Q.DocumentID
            and IR.revision = Q.Revision
